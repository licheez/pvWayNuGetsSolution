...
await dbSemaphoreService.ReleaseSemaphoreAsync(resourceName);

// Acquire the semaphore
// the semaphore was release in the above line
// as such acquiring the semaphore should succeed
await dbSemaphoreService.AcquireSemaphoreAsync(resourceName,
...
// last update date
await dbSemaphoreService.TouchSemaphoreAsync(resourceName);

// let's display the semaphore values
var semaphore = await dbSemaphoreService.GetSemaphoreAsync(resourceName);
Console.WriteLine($"semaphore {semaphore!.Owner} " +
                  $"last updated {semaphore.LastTouchUtcDate}");
